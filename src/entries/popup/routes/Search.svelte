    ...
    variables: {
      ...
      format:
        selectedMode === "ALL"
          ? undefined
          : selectedMode === "MANGA"
            ? [MediaFormat.MANGA, MediaFormat.ONE_SHOT]
            : selectedMode === "LIGHT_NOVEL"
              ? [MediaFormat.NOVEL]
              : undefined,
    },
    pause: searchText === "",
...
<div class="w-full h-full flex flex-col items-center">
  <div class="relative w-2/3">
    ...
    {#if $search.fetching}
      <Icon
        class="absolute right-2 bottom-3 pointer-events-none text-accent"
        icon={faSpinner}
        spin
      />
    {/if}
  ...
  <div class="w-2/3 mt-2 grid grid-cols-4 rounded-lg p-1 gap-0.5 bg-foreground">
    <button
      ...
      on:click={() => (selectedMode = "ALL")}>All</button
    >
    <button
      class="px-2 py-0.5 text-sm rounded hover:bg-accent/40 hover:text-text-400 transition-colors"
      class:selected={selectedMode === "ANIME"}
      on:click={() => (selectedMode = "ANIME")}>Anime</button
    >
    <button
      class="px-2 py-0.5 text-sm rounded hover:bg-accent/40 hover:text-text-400 transition-colors"
      class:selected={selectedMode === "MANGA"}
      on:click={() => (selectedMode = "MANGA")}>Manga</button
    >
    <button
      class="px-2 py-0.5 text-sm rounded hover:bg-accent/40 hover:text-text-400 transition-colors"
      class:selected={selectedMode === "LIGHT_NOVEL"}
      on:click={() => (selectedMode = "LIGHT_NOVEL")}>Novel</button
    >
  </div>
  {#if $search.fetching}
    <Loader />
  ...
  {:else if $search.data}
    <div class="w-full mt-6 flex flex-col space-y-4">
      ...
      {:else}
        <SearchResults
          title="Anime"
          results={$search.data.Page.media.filter(
            (m) => m.type === MediaType.ANIME
          )}
        />
        <SearchResults
          title="Manga"
          results={$search.data.Page.media.filter(
            (m) => m.type === MediaType.MANGA && m.format !== MediaFormat.NOVEL
          )}
        />
        <SearchResults
          title="Light Novels"
          results={$search.data.Page.media.filter(
            (m) => m.format === MediaFormat.NOVEL
          )}
        />
      {/if}
    </div>
